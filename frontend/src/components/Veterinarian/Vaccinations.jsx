...
const Vaccinations = () => {
  ...
  useEffect(() => {
    ...
    // Fetch vet's appointments to use as a patient list
    axios.get(`${API}/api/appointments`, { headers: { Authorization: `Bearer ${token}` } })
      .then(res => setAppointments(res.data))
      .catch(err => console.error(err))
      .finally(() => setLoading(false));

    // Fetch available vaccine catalog
  ...
  const handleSelectPatient = async (appt) => {
    // We group by pet, so just take the pet data
    setSelectedPet({ petId: appt.petId, petName: appt.petName, petSpecies: appt.petSpecies, ownerName: appt.ownerName });
    
    const token = localStorage.getItem('chivas_token');
    try {
        const res = await axios.get(`${API}/api/vaccinations/pet/${appt.petId}`, { 
  ...
  // Group unique patients from appointments
